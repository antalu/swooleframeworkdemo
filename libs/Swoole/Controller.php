...
class Controller extends Object
{
    ...
    * @创建者： xiongjianbang
    * @作　用： 业务逻辑的日志
    * @create:  2016-11-3 上午11:25:08
    */
    function log($msg=''){
    	if (defined('DEBUG') and DEBUG == 'on'){
    		$obj = new \Swoole\Log\FileLog(\Swoole::$app_path . '/web_business.log');
    		$obj->put($msg);
    	}
    }
    ...
    * @param：$arr = 数组
    * @return： 加密后的字符串
    * @create:  2016-11-3 下午2:41:58
    */
    public function rsa_encode($arr = array()){
    	if(empty($arr)){
    		return $arr;
    	}
    	$rsa = new \Swoole\Rsa();
    	...
    	unset($rsa,$arr,$str_get);
    	return $encode;
    }
    
    public function rsa_decode($str_encode = ''){
    	if(empty($str_encode)){
    		return $str_encode;
    	}
    	$rsa = new \Swoole\Rsa();
    	...
    	unset($rsa,$str_encode,$json_decode);
    	return $arr;
    }
